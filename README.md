...
这是一个由网关设计本身带来的、**无法规避**的问题，不是 bug。

多轮对话里，无图轮次的缓存命中率可以稳定在 97% 以上（deepseek 前缀缓存正常命中历史）。但**带图的轮次命中率会明显偏低**，原因有两层：

1. **看图本身的必要成本**：有图时，网关会向 deepseek 的请求末尾追加两段全新内容——VLM 描述（`merged_blocks`）和正文系统提示（`LLM_SYSTEM`），合计约 1500+ token。这两段是每轮重新生成、天然无法命中缓存的，属于"给纯文本模型转述图片"必须付的 token 成本。

2. **输出拼接造成的前缀错位**：deepseek 自己生成的思考（`reasoning_content` raw）里并不含 VLM 描述；但网关为了把图片信息带给客户端，会把 VLM 描述作为 `thinking` 块**拼接**进 assistant 的响应。于是客户端拿到的 assistant 思考内容是 `VLM 描述 + 真实思考`，而 deepseek 落盘缓存单元里只有 `真实思考`。下一轮客户端回传这段历史时，从思考内容的第一个 token 起前缀就对不上，导致**上一轮有图轮的整条 output message 完全 miss**。

这两层叠加，使有图轮及其后一轮的命中率都低于无图轮。miss 的 token 分两部分：一是有图轮新增的 VLM 描述 + 正文系统提示（约 1500+ token，相对恒定）；二是有图轮的整条 output message（真实思考 + 正文回复）在下一轮因前缀错位而完全 miss，这部分大小随模型当轮输出长度变化，并不恒定。但两者都是**一次性的**——只影响有图轮和有图轮之后那一轮，不会随对话轮次累积。所以上下文窗口越长、历史越厚，这些一次性 miss 的相对占比就越小，对长对话影响有限，对"每轮都带图"的短对话场景最明显。

如果这个命中率损耗对某个场景不可接受，唯一的根本解法是改用原生多模态模型（图片不再需要文字转述），而不是在这个网关架构内优化——因为"前置 VLM 描述 + 后置 LLM"这个模式决定了图片信息必然要以一段每轮变化的文字进入上下文。
