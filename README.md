...
**第一步，看整体。** VLM-1 只对着图片，先判断画面类型再按对应结构输出整体描述。它分四类：screen（界面、文档、表格、代码——输出布局、元素、文字、结构）、chart（图表、流程图——输出坐标轴、图例、关键数据）、scene（照片、人物、实物——输出构图、主体、背景、光影、质感、氛围）、other（抽象图形——输出主体、联想）。这一步对应人扫一眼"这是什么场景"。

**第二步，带着问题细看。** VLM-2 同时拿到图片和用户的问题，只输出与问题强相关的聚焦描述——用户问"这个按钮为什么点不了"，它就描述按钮的状态、周边提示、关联表单，而不是把整张图再复述一遍。注意：VLM-2 永远只输出对画面的描述，绝不回答问题——它的输出是"观察/推断/无法确认"式的画面信息，回答这件事始终由 deepseek 完成。

为什么拆两步，而不是一个 VLM 全量描述？因为"描述全图 + 回答问题"是两件互相干扰的事：全量描述的输出 token 是最大成本项，而且模型一边报流水账一边找答案，容易顾此失彼。拆开之后，VLM-1 专心输出整体结构，VLM-2 专心提取问题相关的画面信息，聚焦通道反而能真正盯住用户关心的部分——它输出的不是答案，而是"带着倾向的描述"，让 deepseek 有足够的画面信息去感知和决策。

两路描述怎么合？字段不相交，直接拼接——VLM-1 管整体、VLM-2 管聚焦，冲突以 VLM-2 为准，最后把用户问题一起交给 deepseek。deepseek 拿到的不是一段"图片文字版"，而是"整体描述 + 聚焦描述 + 原问题"的完整上下文。
